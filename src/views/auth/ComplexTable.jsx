...
const ComplexTableContests = ({ columnsData, tableData: initialTableData }) => {
  ...
  const [selectedContest, setSelectedContest] = useState(() => {
    const savedContest = localStorage.getItem('selectedContest');
    return savedContest ? JSON.parse(savedContest) : null;
  });

  useEffect(() => {
    if (selectedContest) {
      localStorage.setItem('selectedContest', JSON.stringify(selectedContest));
    } else {
  ...
  const handleRowClick = (contest) => {
    ...
    console.log(`Row clicked, contest ID: ${contest._id}`);
  };
  ...
  if (selectedContest) {
    return <ContestDetails contest={selectedContest} onBack={handleBack} />;
  }
